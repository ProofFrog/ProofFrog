...
class EcFile:
    ...
    `abstract_requires` is for theories that must be brought into scope WITHOUT
    importing their names -- an abstract theory meant to be cloned, such as
    `BitWord`, whose `n` / `word` would collide at top level. It renders as a
    separate `require <names>.` line after the import line.
    ...
    abstract_requires: list[str] = field(default_factory=list)
...
def pretty_print(ec_file: EcFile) -> str:
    ...
    if ec_file.abstract_requires:
        lines.append(f"require {' '.join(ec_file.abstract_requires)}.")
    lines.append("")
