(* hop_2 challenge -- FAITHFUL wrapper shape (the exact real export structure):
   - un-inlined challengers (LHS then -> KEM_PQ Unbreakable binding challenger with
     its own dk0/dk1 fields, coupled; RHS else -> KDF Breakable, stateless) needing
     inline{i} 1 in the tactic;
   - `ct_T <- ct.2` projections in the prefixes (LHS 18 stmts, RHS 20 stmts) needing
     `sp` before functionalization.
   Core math identical to binding_challenge_hop2_casesplit.ec (validated). *)
...
module UnbrPQ (P:KP) : PQBind = {
  var dk0 : dkpq  var dk1 : dkpq
  proc challenge(a b : ctpq) : bool = {
    var s0, s1 : sspq;
    s0 <@ P.dpq(UnbrPQ.dk0, a);
    s1 <@ P.dpq(UnbrPQ.dk1, b);
    return false;
  }
}.
...
module RP (P:KP, T:KT, L:LL, H:HH, CH:PQBind) = {
...
module RK (P:KP, T:KT, L:LL, H:HH, CH:KDFColl) = {
  var dp0 : dkpq  var dt0 : dkt  var ek0 : ekt
  var dp1 : dkpq  var dt1 : dkt  var ek1 : ekt
  proc challenge(ct0 ct1 : ctpq*ctt) : bool = {
    ...
    sp0 <@ P.dpq(RK.dp0, ct_PQ_0); st0 <@ T.dt(RK.dt0, ct_T_0);
    a0 <@ P.epq_(sp0); a1 <@ T.et_(st0); a2 <@ T.ect_(ct_T_0); a3 <@ T.eek_(RK.ek0); a4 <@ L.get();
    kdf_in_0 <- c4 (c3 (c2 (c1 a0 a1) a2) a3) a4;
    ct_PQ_1 <- ct1.`1; ct_T_1 <- ct1.`2;
    sp1 <@ P.dpq(RK.dp1, ct_PQ_1); st1 <@ T.dt(RK.dt1, ct_T_1);
    e0 <@ P.epq_(sp1); e1 <@ T.et_(st1); e2 <@ T.ect_(ct_T_1); e3 <@ T.eek_(RK.ek1); e4 <@ L.get();
...
section.
  declare module P <: KP {-RP, -RK, -UnbrPQ}.
  ...
  lemma hop2_elim :
    equiv [ RP(P,T,L,H,UnbrPQ(P)).challenge ~ RK(P,T,L,H,BrkKDF(H)).challenge :
       ...
       RP.dp0{1}=UnbrPQ.dk0{1} /\ RP.dp1{1}=UnbrPQ.dk1{1}
       ==> ={res} ].
  proof.
    proc.
    seq 18 20 : (={ct0, ct1} /\ kdf_in_0{1} = kdf_in_0{2} /\ kdf_in_1{1} = kdf_in_1{2} /\
                 ...
                 RP.dp0{1}=UnbrPQ.dk0{1} /\ RP.dp1{1}=UnbrPQ.dk1{1} /\
                 RP.dp0{1}=RK.dp0{2} /\ RP.dt0{1}=RK.dt0{2} /\ RP.dp1{1}=RK.dp1{2} /\ RP.dt1{1}=RK.dt1{2} /\
    ...
    + sp.
      ...
      wp.
      call{2} (get_det gl). call{2} (eek_det gt rek1). call{2} (ect_det gt lc1.`2).
      call{2} (et_det gt (ev_dt rdt1 lc1.`2)). call{2} (epq_det gp (ev_dpq rdp1 lc1.`1)).
      call{2} (dt_det gt rdt1 lc1.`2). call{2} (dpq_det gp rdp1 lc1.`1).
      call{1} (get_det gl). call{1} (eek_det gt lek1). call{1} (ect_det gt lc1.`2).
      call{1} (et_det gt (ev_dt ldt1 lc1.`2)). call{1} (dt_det gt ldt1 lc1.`2).
      call{1} (epq_det gp (ev_dpq ldp1 lc1.`1)). call{1} (dpq_det gp ldp1 lc1.`1).
      wp.
      call{2} (get_det gl). call{2} (eek_det gt rek0). call{2} (ect_det gt lc0.`2).
      call{2} (et_det gt (ev_dt rdt0 lc0.`2)). call{2} (epq_det gp (ev_dpq rdp0 lc0.`1)).
      call{2} (dt_det gt rdt0 lc0.`2). call{2} (dpq_det gp rdp0 lc0.`1).
      call{1} (get_det gl). call{1} (eek_det gt lek0). call{1} (ect_det gt lc0.`2).
      call{1} (et_det gt (ev_dt ldt0 lc0.`2)). call{1} (dt_det gt ldt0 lc0.`2).
      call{1} (epq_det gp (ev_dpq ldp0 lc0.`1)). call{1} (dpq_det gp ldp0 lc0.`1).
      skip => />.
    case (ct0{2} = ct1{2}).
    ...
    + rcondt{1} 1; first by auto.
      inline{1} 1.
      sp.
      exists* (glob P){1}, UnbrPQ.dk0{1}, UnbrPQ.dk1{1}, ct0{1}, ct1{1}; elim* => gp3 xdp0 xdp1 xc0 xc1.
