...
- `proof_frog/visitors.py` — AST visitor/transformer base classes (`Visitor[U]`, `Transformer`, `BlockTransformer`) and core utility visitors/transformers (substitution, inlining, Z3/SymPy conversion, type maps). **Capture-aware renaming:** `frog_ast.Variable` is declared `class Variable(Expression, Type)`, so one node class models both a value reference and a user-defined *type* name (a `let:`-bound `Set`, a `Set Key = ...` alias). A `SubstitutionTransformer` keyed on a bare `Variable(name)` therefore rewrites type positions too, which silently retypes a game. When the intent is to rename a **value** (a field, a local), use `rename_value_references(node, {old: new})` instead — it consults `TYPE_SLOTS` / `EXPRESSION_ESCAPE_SLOTS` so type positions are skipped while length/size arguments (`BitString<n>`) stay renameable — and guard the minted names with `type_position_names(game)` so a canonical name can never fuse with an in-scope type name
