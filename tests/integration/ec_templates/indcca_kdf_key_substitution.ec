...
(*                                                               *)
(* WHAT THIS TRIPWIRE DOES NOT COVER, measured against the real  *)
(* flat states AFTER it was written: the real hop's two KDF       *)
(* inputs are the same five pieces under DIFFERENT concat         *)
(* nestings --                                                    *)
(*   L: C4(C3(C2(C1(enc_k, ss_T), e_ct), e_ek), label)            *)
(*   R: CX(challenger_k, D3(D2(D1(ss_T, e_ct), e_ek), label))     *)
(* -- equal by ASSOCIATIVITY, for which the exporter emits no law  *)
(* (only the round-trip family). This file uses a single `concat`  *)
(* op and so sidesteps it. It therefore validates the REORDER and  *)
(* the BIJECTION COUPLING, not the whole hop.                      *)
(*                                                               *)
