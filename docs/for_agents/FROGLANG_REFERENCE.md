...
- **`requires` clauses** (optional): Preconditions on parameters that must hold for the scheme to be well-defined (e.g., `requires F.out >= lambda;`). `requires` may express type-level equalities (`requires E.Key == BitString<n>;`) or value-level numeric equalities (`requires F.in == 1;`); the latter propagate into typechecking, so a literal `BitString<1>` can unify with a symbolic `BitString<F.in>` under such a constraint.
