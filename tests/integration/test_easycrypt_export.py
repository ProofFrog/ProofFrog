...
def test_ies_delta_walk_template_compiles(tmp_path: Path) -> None:
    ...
    in the template header. If this stops compiling, re-derive the walk
    before trusting ``_ies_delta_walk_step``."""
    ec_file = tmp_path / "ies_delta_walk.ec"
    ec_file.write_text(IES_DELTA_WALK_TEMPLATE.read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
        capture_output=True,
        text=True,
...
@pytest.mark.parametrize(
    "template",
    ["fold_pair_walk.ec", "det_tail_fold.ec"],
)
def test_if_fold_templates_compile(tmp_path: Path, template: str) -> None:
    ...
    deterministic-tail (Absorb) row. Both templates were GENERATED by
