...
def test_str_roundtrip_preserves_helpers_before() -> None:
    ast = _parse(_HELPER_GAME + _PROOF_BLOCK)
    ast2 = _parse(str(ast))
    assert len(ast2.helpers) == 1
    assert ast2.helpers_after_theorem_count == 0


def test_str_roundtrip_preserves_helpers_split() -> None:
    helper_a = _HELPER_GAME.replace("Dummy", "DummyA")
    helper_b = _HELPER_GAME.replace("Dummy", "DummyB")
    ast = _parse(helper_a + _PROOF_BLOCK + helper_b)
    ast2 = _parse(str(ast))
    assert len(ast2.helpers) == 2
    assert ast2.helpers_after_theorem_count == 1
    assert ast2.helpers[0].name == "DummyA"
    assert ast2.helpers[1].name == "DummyB"


def test_str_roundtrip_idempotent_across_layouts() -> None:
    """Three-way layout symmetry: parse → unparse → parse → unparse is a
    fixed point for each of the before / after / split layouts."""
    helper_a = _HELPER_GAME.replace("Dummy", "DummyA")
    helper_b = _HELPER_GAME.replace("Dummy", "DummyB")
    for src in [
        _HELPER_GAME + _PROOF_BLOCK,
        _PROOF_BLOCK + _HELPER_GAME,
        helper_a + _PROOF_BLOCK + helper_b,
    ]:
        text1 = str(_parse(src))
        text2 = str(_parse(text1))
        assert text1 == text2
