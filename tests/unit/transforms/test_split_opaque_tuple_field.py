...
class TestSplitOpaqueTupleField:
    ...
    def test_negative_initialize_has_early_return_in_if(self) -> None:
        """Initialize containing a nested ``ReturnStatement`` (inside an if):
        splitting would leave per-index fields uninitialized on the
        early-return path. The pass must decline. Not surface-reachable
        through the parser (Initialize is Void), but constructible at AST
        level. Mirrors the equivalent guard on
        ``HoistDeterministicCallToInitialize`` /
        ``HoistGroupExpToInitialize`` (April 2026 a78a070 / 2ab448f pattern).
        """
        game = frog_parser.parse_game("""
            Game Foo() {
                [BitString<n>, BitString<n>] keys;
        ...
        init.block = frog_ast.Block(
            [early_return_if] + list(init.block.statements)
        )
        result = SplitOpaqueTupleFieldTransformer().transform(game)
        assert result == game
