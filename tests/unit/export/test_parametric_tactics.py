...
def test_modint_tactic_add_left() -> None:
    """``k + m -> k`` synthesizes an rnd over the stdlib ZModRing ops."""
    add = frog_ast.BinaryOperation(
        frog_ast.BinaryOperators.ADD,
        frog_ast.Variable("k"),
        frog_ast.Variable("m"),
    )
    tac = parametric_tactics.uniform_modint_tactic(_app(add), TypeCollector())
    assert tac is not None
    body = "\n".join(tac)
    assert (
        "rnd (fun z => ModInt_q.( + ) z m{2}) (fun z => ModInt_q.( - ) z m{2})." in body
    )
    # Discharge: bijection by ring (@ModInt_q), support by the uniform distr.
    assert "ModInt_q.DZmodP.dunifin_fu" in body
    assert "ModInt_q.DZmodP.dunifin_funi" in body
    assert "@ModInt_q" in body


def test_modint_tactic_add_left_reversed() -> None:
    """The reversed micro (``state_{k+1} ~ state_k``) swaps the bijection.

    Unlike XOR (self-inverse, so fwd == inv), the ring's ``+``/``-`` are
    distinct: the reversed direction must emit ``-`` first with ``+`` as its
    inverse, or EC's ``rnd`` side-goal is false.
    """
    add = frog_ast.BinaryOperation(
        frog_ast.BinaryOperators.ADD,
        frog_ast.Variable("k"),
    ...
    tac = parametric_tactics.uniform_modint_tactic(
        _app(add), TypeCollector(), reversed_dir=True
    )
    assert tac is not None
    assert (
        "rnd (fun z => ModInt_q.( - ) z m{2}) (fun z => ModInt_q.( + ) z m{2})."
        in "\n".join(tac)
    )


def test_modint_tactic_sub_left() -> None:
    """``k - m -> k`` synthesizes an rnd with ``-`` first, ``+`` as inverse."""
    sub = frog_ast.BinaryOperation(
        frog_ast.BinaryOperators.SUBTRACT,
        frog_ast.Variable("k"),
        frog_ast.Variable("m"),
    )
    tac = parametric_tactics.uniform_modint_tactic(_app(sub), TypeCollector())
    assert tac is not None
    assert (
        "rnd (fun z => ModInt_q.( - ) z m{2}) (fun z => ModInt_q.( + ) z m{2})."
        in "\n".join(tac)
...
def test_modint_tactic_minus_uniform_bails() -> None:
    ...
    assert parametric_tactics.uniform_modint_tactic(_app(sub), TypeCollector()) is None
