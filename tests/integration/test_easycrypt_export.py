...
IES_DELTA_WALK_TEMPLATE = EC_TEMPLATES / "ies_delta_walk.ec"
HOIST_PAIR_WALK_TEMPLATE = EC_TEMPLATES / "hoist_pair_walk.ec"
FOLD_PAIR_WALK_TEMPLATE = EC_TEMPLATES / "fold_pair_walk.ec"
DET_TAIL_FOLD_TEMPLATE = EC_TEMPLATES / "det_tail_fold.ec"
RENDERED_IDENTITY_TEMPLATE = EC_TEMPLATES / "rendered_identity.ec"
ISUV_ALIGN_WALK_TEMPLATE = EC_TEMPLATES / "isuv_align_walk.ec"
...
@pytest.mark.parametrize(
    "template",
    ["ies_delta_walk.ec", "ies_delta_walk_canonical.ec"],
)
def test_ies_delta_walk_template_compiles(tmp_path: Path, template: str) -> None:
    """Regression tripwire for the Move 3a IES delta-det guard-site walk
    (Phase-2 micro synthesizers). The template was GENERATED by
    ``_ies_delta_walk_step`` itself over a mini state_20/21-shaped pair
    rendered by the real renderer, and its tactic text is asserted in
    lockstep with the synthesizer by
    ``tests/unit/export/test_ies_delta_walk.py``. The negative control
    (falsified guard) EC-rejects at the if guard-equality subgoal — recorded
    in the template header. The ``_canonical`` variant is the same pair
    rendered under the chain-wide canonical ``f<NN>`` field naming a
    random-oracle proof uses: the route renders its own copy of each state
    to read the bodies its tactic names, and rendering that copy under a
    different field-naming decision than the emitted module made every
    field pin name a variable that does not exist. If either stops
    compiling, re-derive the walk before trusting
    ...
    assert result.returncode == 0, (
        f"EasyCrypt rejected the IES delta-walk template.\n"
        f"stderr:\n{result.stderr}\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
...
def test_hoist_pair_walk_template_compiles(tmp_path: Path) -> None:
    """Regression tripwire for the Move 3c Hoist-pair micro walk (Phase-2
    micro synthesizers). The template was GENERATED by the synthesizer
    itself (``_detect_hoist_pair`` / ``_hoist_conjunct_registry`` /
    ``_hoist_pair_step`` / ``_make_field_aware_coupling`` /
    ``_render_coupling_chain_body``) over a mini Hoist pair rendered by the
    real renderer, and its tactic text is asserted in lockstep with the
    synthesizer by ``tests/unit/export/test_hoist_pair_walk.py``. The
    negative control (falsified cache conjunct) EC-rejects at the consumer's
    closing ``/#`` — recorded in the template header. If this stops
    compiling, re-derive before trusting ``_hoist_pair_step``."""
    ec_file = tmp_path / "hoist_pair_walk.ec"
    ec_file.write_text(HOIST_PAIR_WALK_TEMPLATE.read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0, (
        f"EasyCrypt rejected the Hoist-pair walk template.\n"
        f"stderr:\n{result.stderr}\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
...
@pytest.mark.parametrize(
    "template",
    ["fold_pair_walk.ec", "det_tail_fold.ec", "fold_pair_walk_canonical.ec"],
)
def test_if_fold_templates_compile(tmp_path: Path, template: str) -> None:
    """Regression tripwires for the Move 4 if-fold rows (Phase-2 micro
    synthesizers): the Fold Equivalent Return Branch case tree and the
    deterministic-tail (Absorb) row, plus the same fold pair under the
    chain-wide canonical ``f<NN>`` field naming a random-oracle proof uses
    (``fold_pair_walk_canonical.ec`` — the route renders its own copy of
    each state to read the bodies its tactic names, and rendering that
    copy under a DIFFERENT field-naming decision than the emitted module
    made every field pin name a variable that does not exist). Both
    templates were GENERATED by
    ``_if_fold_step`` over mini pairs rendered by the real renderer, and
    their tactic text is asserted in lockstep with the synthesizer by
    ``tests/unit/export/test_if_fold_walk.py``. The negative controls
    (falsified then-branch X; dropped ``!P`` guard conjunct) EC-reject at
    the P-true smt / the closing ``/#`` — recorded in the template
    headers. If either stops compiling, re-derive before trusting
    ``_if_fold_step``."""
    ec_file = tmp_path / template
    ec_file.write_text((EC_TEMPLATES / template).read_text())
    result = subprocess.run(
    ...
    assert result.returncode == 0, (
        f"EasyCrypt rejected the {template} template.\n"
        f"stderr:\n{result.stderr}\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
...
def test_same_memory_coupling_template_compiles(tmp_path: Path) -> None:
    ...
    a coupling that also carries a one-memory conjunct
    (``S.f09{1} = S.f08{1}`` -- the survivor-consistency facts a broken chain
    threads) defeats it: *cannot infer the set of equalities*, measured on
    ``CK_seedbased_LEAK_BIND_K_PK`` ``micro_2_hashg_left_22``. The template
    was GENERATED by ``_crossed_coupling_sim_step`` over a mini pair rendered
    by the real renderer; its tactic is asserted in lockstep with the
    synthesizer by ``tests/unit/export/test_same_memory_coupling.py``. Two
    controls are recorded in the template header: restoring ``proc; sim.``
    reproduces the original error, and dropping a load-bearing conjunct from
    the precondition gives ``cannot prove goal (strict)``."""
    ec_file = tmp_path / "same_memory_coupling.ec"
    ec_file.write_text((EC_TEMPLATES / "same_memory_coupling.ec").read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0, (
        f"EasyCrypt rejected the same-memory-coupling template.\n"
        f"stderr:\n{result.stderr}\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
...
def test_rendered_identity_template_compiles(tmp_path: Path) -> None:
    ...
    (the measured ``Symbolic Computation`` width-annotation class). The
    template was GENERATED by ``_rendered_identity_step`` over a mini
    width-annotation pair rendered by the real renderer; its tactic is
    asserted in lockstep with the synthesizer by
    ``tests/unit/export/test_rendered_identity.py``. The negative control
    (making the two modules genuinely differ) leaves ``sim``'s goal open
    and EC refuses to save the proof — recorded in the template header."""
    ec_file = tmp_path / "rendered_identity.ec"
    ec_file.write_text(RENDERED_IDENTITY_TEMPLATE.read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0, (
        f"EasyCrypt rejected the rendered-identity template.\n"
        f"stderr:\n{result.stderr}\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
...
def test_isuv_align_walk_template_compiles(tmp_path: Path) -> None:
    """Regression tripwire for the Move 6 ISUV calls-alignment row (Phase-2
    micro synthesizers): an inlining step that also exposed an independent
    cross-module call reorder. The template was GENERATED by
    ``_isuv_align_step`` over a mini inlining pair rendered by the real
    renderer; its tactic is asserted in lockstep with the synthesizer by
    ``tests/unit/export/test_isuv_align.py``. Both negative controls are
    proof-level: dropping the alignment swaps yields "K.ess and N.ets
    should be equal"; falsifying an untouched-field frame conjunct yields
    "cannot prove goal (strict)" — recorded in the template header."""
    ec_file = tmp_path / "isuv_align_walk.ec"
    ec_file.write_text(ISUV_ALIGN_WALK_TEMPLATE.read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0, (
        f"EasyCrypt rejected the ISUV align-walk template.\n"
        f"stderr:\n{result.stderr}\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
...
def test_plain_plumbing_peel_template(tmp_path: Path) -> None:
    """The tactic the plain deterministic-plumbing peel emits, as EasyCrypt
    checks it.

    The two modules are a flat-state pair whose abstract-call backbone is
    identical and whose bodies differ only in deterministic plumbing (a
    repeated use extracted into a local). ``sim`` cannot close that -- it
    aligns the extra assignment against the first call -- while ``wp``
    absorbs the deterministic runs and ``/#`` equates the substituted
    expressions. The proof body is generated by the synthesizer over the
    matching mini game pair and pinned in lockstep by
    ``tests/unit/export/test_plain_plumbing_peel.py``.
    """
    template = "plain_plumbing_peel.ec"
    ec_file = tmp_path / template
    ec_file.write_text((EC_TEMPLATES / template).read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
...
def test_plain_plumbing_peel_falsified_rejects(tmp_path: Path) -> None:
    """A REJECTING template: EasyCrypt must REFUSE it, and that is the point.

    The same tactic as the accepting sibling, against a body whose second
    call takes the WRONG key, so the closer is handed ``dk1{1} = dk0{2}``
    from a precondition offering ``dk1{1} = dk1{2}``. A tactic that RUNS is
    not evidence that it did any work; this file is what makes the accepting
    template mean something.
    """
    template = "plain_plumbing_peel_falsified_rejects.ec"
    ec_file = tmp_path / template
    ec_file.write_text((EC_TEMPLATES / template).read_text())
    result = subprocess.run(
        ["bash", str(EC_SCRIPT), str(ec_file)],
    ...
    assert result.returncode != 0, (
        "EasyCrypt ACCEPTED the falsified plumbing peel; the row is closing "
        "goals it should not.\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
    assert "cannot prove goal" in (result.stdout + result.stderr), (
        "EasyCrypt rejected the control for a different reason than the one "
        "it documents.\n"
        f"stdout:\n{result.stdout[-2000:]}"
    )
